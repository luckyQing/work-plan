...
public class ScheduleService {
     ...
     * 根据查询条件确定人员范围
     */
    private List<UserEntity> resolveUsers(ScheduleReqVO reqVO, Long selfUserId) {
        if (reqVO.isOnlySelf() && selfUserId != null) {
            UserEntity self = userService.getById(selfUserId);
    ...
    private Set<Long> resolveDemandIdsByProject(Long projectId) {
        ...
        List<DemandEntity> demands = demandService.list(
                new LambdaQueryWrapper<DemandEntity>().eq(DemandEntity::getProjectId, projectId));
        return demands.stream().map(DemandEntity::getId).collect(Collectors.toSet());
    }
    ...
    private ScheduleTaskRespVO buildDemandTaskRespVO(DemandEntity demand, List<TaskDO> subTasks, ScheduleReqVO reqVO) {
        ...
        BigDecimal totalH = subTasks.stream().map(TaskEntity::getTotalHours).reduce(BigDecimal.ZERO, BigDecimal::add);
